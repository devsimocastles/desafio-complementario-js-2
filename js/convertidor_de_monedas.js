...
let lista_de_operaciones = [];
let contador_operaciones = 1;

function agregar_operacion(mensaje, lista_de_operaciones){
    let operacion = {};
    operacion["ID"] = contador_operaciones;
    operacion["Operacion"] = mensaje;
    lista_de_operaciones.push(operacion)
...
function convertir_pesos_a_moneda( cantidad_pesos, moneda_a_convertir , cambio ) {
    ...
    agregar_operacion(mensaje, lista_de_operaciones)
    return mensaje
}
...
while (true){
    ...
    11 - Mostrar historial de Operaciones
...
Escriba la cantidad de Real Brasileño que desea convertir a Peso Argentino:
        ...
        if ( lista_de_operaciones.length != 0 ){
            let operaciones_str = "";
            for( let operacion of lista_de_operaciones){
                operaciones_str += `Numero de Operación: ${operacion.ID}, Operación: ${operacion.Operacion}\n`;
            }
            alert(operaciones_str);
        } else {
            alert("No ha realizado ninguna operación.");
            continue
        }
    }
